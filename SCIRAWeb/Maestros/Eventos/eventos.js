...
function inicializarVentana() {
    armarDataTableLista([]);
    armarDataTableConResidentes([]);
    armarDataTableConTorres([]);
    listar();
...
function listar() {
    ...
    $.ajax({
        type: "GET",
        url: gsUrlApi + "/eventos/listar/" + sIdEmpresa,
...
function guardar() {
    ...
    try {
        ...
        if (ValidarCamposJson("#div_Formulario")) {
            ...
            objeventos.Nombre = $("#input_Nombre").val();
            objeventos.Descripcion = $("#input_Descripcion").val();
            objeventos.Codigo = $("#input_Codigo").val();
            objeventos.Empresa = obtenerQueryString("IdEmpresa");




            if ($("#hidden_Ideventos").val()) {
                sAccion = "actualizar";
                objeventos._id = $("#hidden_Ideventos").val();
            } else {
                sAccion = "insertar";
              
            }

            sDataRol = JSON.stringify(objeventos);

            $.ajax({
                type: "POST",
                url: gsUrlApi + "/eventos/" + sAccion,
                data: sDataRol,
                headers: gsAutenticacion,
                contentType: "application/json; charset=utf-8",
                dataType: "json",
...
function eliminar() {
    ...
    var sIdRegistro = $("#hidden_Ideventos").val();
...
function quitarRegistro(sKey) {
    ...
    var objeventos = new Object();
    objeventos._id = sKey;
    var sDataRol = JSON.stringify(objeventos);

    $.ajax({
        type: "POST",
        url: gsUrlApi + "/eventos/eliminar",
        data: sDataRol,
        headers: gsAutenticacion,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function cargarData(objData) {
    $("#hidden_Ideventos").val(objData._id);
...
function armarDataTableConResidentes(lstData) {

    gObjGrid = $('#table_ConfiguracionResidentes').DataTable({
        data: lstData,
        paging: false,
        ordering: true,
        order: [
        ...
        columns: [{
                ...
                render: function(data, type, row) {
                    var sResult = "";
                    sResult = '<label>' + row.codigoDiagnostico + '</label>';
                    return sResult;
                },
                orderable: false,
                className: 'text-center',
                ...
                render: function(data, type, row) {
                    var sResult = "";
                    sResult = '<label>' + row.nombreDiagnostico + '</label>';
                    return sResult;
                },
                orderable: false,
                className: 'text-center',
                ...
                render: function(data, type, row) {
                    var sResult = "";
                    sResult = '<span class="CursorPointer" onclick="eliminarRango(this);" title="Editar rango"><i class="fa fa-2x fa-minus-circle"></i></span>';
                    return sResult;
                },
                orderable: false,
                className: 'text-center',
...
function armarDataTableConTorres(lstData) {

    gObjGrid = $('#table_ConfiguracionTorres').DataTable({
        data: lstData,
        paging: false,
        ordering: true,
        order: [
